<script>
  ...
  import { selectedRegion, selectedDistrict, calculateStats, interpolateColor, regionAnimationData, surveysRaw, excludedQ, regionToSvgMap } from "$lib/store.js";
  ...
  $effect(() => {
    filteredData.forEach(q => {
      if (q.selected !== q.previousSelected) {
        handleAnswerToggle(q);
        q.previousSelected = q.selected;
      }
    });
  });
  ...
  function handleSchoolToggle(school, event) {
    // Уникаємо подвійного виклику
    event.stopPropagation();
    event.preventDefault();

    if (school === '') {
      // Якщо натиснули "Усі школи" - очищуємо фільтр школи
      FILTERS = { ...FILTERS, 'Школа': [] };
    } else {
      const currentSchools = FILTERS['Школа'] || [];

      const schoolIndex = currentSchools.indexOf(school);

      let newSchools;
      if (schoolIndex > -1) {
        // Якщо школа вже вибрана - видаляємо її
        newSchools = currentSchools.filter(s => s !== school);
      } else {
        // Якщо школа не вибрана - додаємо її
        newSchools = [...currentSchools, school];
      }
      FILTERS = { ...FILTERS, 'Школа': newSchools };
    }
  }
    ...
    {#if !hasDistricts}
      <button class="remove-button" onclick={() => removeRegion(region)} title="Видалити область">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
          <path stroke="white" stroke-width="2" stroke-linecap="round" d="M4 4l8 8M12 4l-8 8"/>
        </svg>
      </button>
    {/if}
          ...
          {#each $selectedDistrict.filter(d => d.startsWith(region + ":")) as districtKey}
            ...
            <div class="district-item">
              ...
              <div class="district">
                ...
                <!-- Список шкіл [Щоб увімкнути цей блок, видаліть цей коментар]
    ...
    <div class="question-section"
      ...
      {#if expandedQs[question]}
        <div class="answers-container">
          {#each filteredData.filter(d => d.question === question) as q (q.id)}
