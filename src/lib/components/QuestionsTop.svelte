<script>
  ...
  import { surveysRaw, priorA } from "$lib/store.js";
  ...
  let chartData = $derived.by(() => {
    ...
    ['Вік', 'Стать'].forEach(question => {
      ...
      // Відображаємо кожну унікальну відповідь та сортуємо їх за порядком в priorA
      const sortedAnswers = Array.from(allPossibleAnswers).sort(
        (a, b) => priorA.indexOf(a) - priorA.indexOf(b)
      );
  ...
  let yScales = $derived(new Map(
    Array.from(groupedData).map(([question, data]) => {
      ...
      // Створення бару висотою від 0 до 100px
        ...
        {#each value as q (q.id)}
          <div class="answer"
            ...
            <div class="bar-wrapper"
              ...
              <!-- Сам бар діаграми -->
              <div class="bar" 
                style="height: {yScales.get(q.question)(q.freq)}px; --bar-width: {yScales.get(q.question)(q.freq)}"></div>
            </div>
...
<style>
  ...
  .bar-text-outside.na.dimmed {
    color: #C8C6C631 !important;
  }
  .bar-text-outside.na.selected {
    color: #999999 !important;
  }
