<script>
  ...
  $effect(() => {
    ...
    filteredData.forEach(q => {
      ...
      // Якщо більше 9% то показувати всередині
      bar.set(q, {
        percentText: newPercentText,
        showOutsideText: percent < 9 || percent === 0
      });
    });
...
<div class="questionContainer">
  {#each Array.from(groupedData).sort(([keyA, dataA], [keyB, dataB]) => {
    // Спочатку сортуємо за кількістю варіантів відповідей (від найменшої)
    const countA = dataA.length;
    const countB = dataB.length;

    if (countA !== countB) {
      return countA - countB;
    }

    // Потім сортуємо за алфавітом
            ...
            {#each sortAnswers(filteredData.filter( d => d.question === key)) as q (q.id)}
              {#key q.id}
                ...
                <div class="horizontal-answer" 
                  ...
                  <!-- Відсоток -->
                  <p class="percent-text" data-freq={q.freq}>{q.percentText}</p>
              ...
              <div class="answer"
                ...
                <div class="bar-wrapper">
                  ...
                  <div class="bar" style="width: {xScales.get(q.question)(q.freq)}px; background-color: {getBarColor(q.answer)};">
                    ...
                    {#if !q.showOutsideText}
                      <p class="percent-text" data-freq={q.freq} transition:fade>{q.percentText}</p>
                    {/if}
                  </div>
                  <!-- Текст зовні бару (якщо бар занадто вузький) -->
                  {#if q.showOutsideText && q.percentText}
                    <p class="bar-text-outside percent-text" data-freq={q.freq} transition:fade>
                      {q.percentText}
                    </p>
                  {/if}
                </div>
...
<style>
  ...
  .answer:hover .percent-text, .horizontal-answer:hover .percent-text {
    visibility: hidden;
  }
  .answer:hover .percent-text::after, .horizontal-answer:hover .percent-text::after {
    position: absolute;
    right: 0;
    content: attr(data-freq);
    visibility: visible;
  }
  .answer:hover .bar-text-outside.percent-text::after, .horizontal-answer:hover .percent-text::after {
    left: 0 !important;
  }
